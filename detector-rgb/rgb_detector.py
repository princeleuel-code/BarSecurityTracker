...
alerts_total = prom.Counter("rgb_alerts_total", "Alerts emitted", ["event"])
infer_latency = prom.Summary("rgb_inference_ms", "Inference latency (ms)")
...
async def detector_loop(ws):
    ...
    try:
        ...
        if not cap.isOpened():
            print(f"🔴 [RGB] Unable to open RTSP: {RTSP_URL}")
...
async def main():
    ...
    async with websockets.serve(detector_loop, "0.0.0.0", WS_PORT, max_queue=4):
        print(f"🎥 RGB detector WebSocket @ :{WS_PORT}")
